# 1 "../src/cancelable_delay.xc"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 141 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../src/cancelable_delay.xc" 2
...
# 11 "../src/cancelable_delay.xc" 2

# 1 "../src/cancelable_delay.h" 1
# 11 "../src/cancelable_delay.h"
typedef interface delay_if {
    [[clears_notification]]
...
[[combinable]]
void server_delay_iff (server delay_if i_delay);
# 13 "../src/cancelable_delay.xc" 2
# 68 "../src/cancelable_delay.xc"
[[combinable]]
void server_delay_iff (server delay_if i_delay) {
    int enabled;
    int do_delay = 0;
    ...
    printf("server_delay_iff started\n");

    while (1) {
        select {

            case i_delay.delay_iff (const unsigned milliseconds_iff):
            {
                if (enabled) {
                    int current_time;
                    tmr :> current_time;
                    timeout = current_time + (milliseconds_iff * ((100U) * 1000U));
                    do_delay = 1;
            ...
            case i_delay.delay_control (const int p_enable):
            {
            ...
            case do_delay => tmr when __builtin_timer_after(timeout) :> void:
            {
