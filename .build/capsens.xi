...
void capsenseInitClock(__clock_t k)
{
  __builtin_stop_clock(k);
  __builtin_set_clk_div(k, 1);
  __builtin_start_clock(k);
}
...
void measureNbit(port cap, unsigned int times[width],
    ...
    asm("setc res[%0], 0" :: "r"(cap));
    asm("setc res[%0], 8" :: "r"(cap));
    cap <: 1 ? ~0 : 0 @ t0;
    t0 += 10;
    cap @ t0 <: 1 ? ~0 : 0;
    __builtin_sync(cap);
    asm("setc res[%0], 0" :: "r"(cap));
    asm("setc res[%0], 8" :: "r"(cap));
    asm("setc res[%0], 0x200f" :: "r"(cap));
    asm("settw res[%0], %1" :: "r"(cap), "r"(32));
    ...
    notSeen = mask;
    curCaps = 1 ? mask : 0;
    curTime = (t1 - t0) & 0xffff;
    for(int i = 0; i < N && notSeen != 0; i++) {
        for(int k = 0; k < 32; k += width) {
            newCaps = (values[i]>>k) & mask;
            newBits = (curCaps^newCaps)&notSeen;
            if (newBits != 0) {
