	...
	.set adc_task.select.y.enable.cases.nstackwords, 0 $M (adc_task.select.y.case.1.nstackwords) $M (adc_task.select.y.case.0.nstackwords) $M (adc_task.select.y.case.3.nstackwords) $M (adc_task.select.y.case.2.nstackwords)
	...
	.set adc_task.select.enable.cases.nstackwords, 0 $M (adc_task.select.case.1.nstackwords) $M (adc_task.select.case.3.nstackwords) $M (adc_task.select.case.2.nstackwords) $M (adc_task.select.case.0.nstackwords)
...
.Ltmp32:
	.size	_i.startkit_adc_acquire_if._chan_y.read, .Ltmp32-_i.startkit_adc_acquire_if._chan_y.read
	.cfi_endproc
...
.Ltmp37:
	.size	_i.startkit_adc_acquire_if._chan_y.trigger, .Ltmp37-_i.startkit_adc_acquire_if._chan_y.trigger
	.cfi_endproc
